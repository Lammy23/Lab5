...
const showResult2D = (Result, containerId, dataArray) => {
...
function addMatrices(matrix1, matrix2){ 
    ...
    if (matrix1.length !== matrix2.length || matrix1[0].length !== matrix2[0].length) {
        printError("ERROR: cannot add matrices, dimensions do not match");
        return;
    }
    ...
    for (let i = 0; i < matrix1.length; i++){
        ...
        for (let j = 0; j < matrix1[0].length; j++){
            addRow.push(matrix1[i][j] + matrix2[i][j]);
        }
        result.push(addRow);
    }
    return result;
...
const subtractMatrices = function (matrix1, matrix2) { 
	// provide the code
    if (matrix1.length !== matrix2.length || matrix1[0].length !== matrix2[0].length) {
        printError("ERROR: cannot subtract matrices, dimensions do not match");
        return;
    }
...
const multiplyMatrices = (matrix1, matrix2) => { 
    ...
    if (!(matrix1[0].length == matrix2.length)){
        printError("ERROR: cannot multiply matrices, dimensions are not compatible");
        return;
    }
